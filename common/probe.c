...
int probe_poll(void)
{
    ...
    if (VPADRead(VPAD_CHAN_0, &vpad, 1, &vpad_error) > 0 &&
        ...
        WHBLogPrintf("AXPROBE VPAD: local buttons ignored; use HOME -> Quitter buttons=0x%08x", vpad.trigger);
    }
    UiInput in;
    memset(&in, 0, sizeof(in));
    ui_poll(&in);
    if (in.quit || (in.pressed & (1u << SDL_CONTROLLER_BUTTON_BACK))) {
        WHBLogPrintf("AXPROBE UI: quit event ignored; use HOME -> Quitter");
    }
    draw();
    return 1;
}
