...
static void draw(void)
{
    ...
    ui_text(30, UI_HEIGHT - 44, UI_SIZE_BODY, UI_TEXT, "HOME menu -> Quitter");
    ui_present();
}
...
int probe_poll(void)
{
    if (!proc_running()) return 0;
    VPADStatus vpad;
    VPADReadError vpad_error = VPAD_READ_SUCCESS;
    memset(&vpad, 0, sizeof(vpad));
    if (VPADRead(VPAD_CHAN_0, &vpad, 1, &vpad_error) > 0 &&
        vpad_error == VPAD_READ_SUCCESS &&
        (vpad.trigger & (VPAD_BUTTON_MINUS | VPAD_BUTTON_PLUS | VPAD_BUTTON_B))) {
        WHBLogPrintf("AXPROBE VPAD: local buttons ignored in HOME overlay mode buttons=0x%08x", vpad.trigger);
    }
    UiInput in;
    memset(&in, 0, sizeof(in));
    ui_poll(&in);
    if (in.quit || (in.pressed & (1u << SDL_CONTROLLER_BUTTON_BACK))) {
        WHBLogPrintf("AXPROBE UI: exit requested (%s)", in.quit ? "SDL quit" : "MINUS");
        proc_stop();
    }
    draw();
    return 1;
...
{
    if (g_up) {
        WHBLogPrintf("AXPROBE UI: shutdown begin");
        ui_shutdown();
        WHBLogPrintf("AXPROBE UI: shutdown end");
        g_up = 0;
    }
